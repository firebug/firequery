...
const InspectorOverlayId = InspectorOverlay.prototype.overlayId;
...
/** @lends FireQueryToolboxOverlay */
{
  ...
  attach: makeInfallible(function() {
    ...
    Rdp.registerTabActor(client, config).then(({registrar, front}) => {
      ...
      this.front.on("data-modified", this.onDataModified);
      ...
      this.patchJQuery().then(() => {
        // Emit an event indicating that the attach process is done.
        // This can be used e.g. by tests.
        emit(this, "attached", front);

        // xxxHonza: InspectorOverlay should register for the attach
        // event, but when? The ToolboxOverlay is created quite late.
        let inspectorOverlay = this.context.getOverlay(InspectorOverlayId);
        inspectorOverlay.onAttach(front);
  ...
  patchJQuery: function() {
    ...
    this.toolbox.initInspector().then(() => {
      ...
      this.front.patchJQuery(patch, watcher, walker.actorID).then(() => {
        deferred.resolve();
      });
    });
  ...
  onDataModified: function(data) {
    Trace.sysout("FireQueryToolboxOverlay.onDataModified;", data);

    let inspectorOverlay = this.context.getOverlay(InspectorOverlayId);
    let markupView = inspectorOverlay.panel.markup;

    let client = this.toolbox.target.client;
    let nodeFront = client.getActor(data.actorID);
