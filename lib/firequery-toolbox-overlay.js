...
/** @lends FireQueryToolboxOverlay */
{
  ...
  attach: makeInfallible(function() {
    ...
    Rdp.registerTabActor(client, config).then(({registrar, front}) => {
      Trace.sysout("ConsoleOverlay.attach; READY", this);
  ...
  onDataModified: function(nodeFront) {
    ...
    FBTrace.sysout("!!!container " + container, container);
    FBTrace.sysout("!!!containers " + container, markupView._containers);
    if (container) {
      container.update();
    }
