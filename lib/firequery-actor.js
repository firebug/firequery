...
/** @lends FireQueryActor */
{
  ...
  patchJQuery: method(expectState("attached", function(patch, watcher, walkerActorID) {
    ...
    win.addEventListener("firequery-event", event => {
      ...
      Trace.sysout("FireQueryActor.fireQueryEvent;", result);

      // xxxHonza: send instance of NodeActor back to the client.
      Events.emit(this, "data-modified", {
        actorID: nodeActor.actorID,
        jQueryData: hasJQueryCache(nodeActor.rawNode)
      });
  ...
  onDomNodeForm: makeInfallible(function(eventId, nodeActor, form) {
    let data = hasJQueryCache(nodeActor.rawNode);
    form.jQueryCacheData = data;
...
function evalJQueryCache(object) {
  try {
    ...
    var idOrCache = data(object.wrappedJSObject || object,
...
// Patching NodeFront to avoid 'ensureParentFront is undefined' exception.
let domNodeType = types.getType("domnode");
let NodeFront = domNodeType.frontClass;

let originalFrontForm = NodeFront.prototype.form;
NodeFront.prototype.form = function(form, detail, ctx) {
  let pool = ctx.marshallPool();
  let parent = form.parent;

  if (!pool.ensureParentFront) {
    form.parent = null;
  }

  originalFrontForm.apply(this, arguments);
